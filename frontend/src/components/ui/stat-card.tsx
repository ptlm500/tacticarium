    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="text-[10px] uppercase tracking-widest text-foreground/60">{title}</div>
          <div className="mt-1 flex items-baseline gap-1.5">
            <span className="font-mono text-2xl font-bold text-foreground tabular-nums">
              {displayValue !== null ? displayValue : value}
            </span>
            {unit && <span className="font-mono text-sm text-foreground/50">{unit}</span>}
          </div>
          {trend && (
            <div
              className={cn("mt-1 flex items-center gap-1 font-mono text-xs", trendColor[trend])}
            >
