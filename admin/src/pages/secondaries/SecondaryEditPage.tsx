...
export function SecondaryEditPage() {
  ...
  const [form, setForm] = useState<Secondary>({ id: '', missionPackId: '', name: '', lore: '', description: '', maxVp: 15, isFixed: false, scoringOptions: [] });
  ...
  useEffect(() => {
    if (id) adminApi.secondaries.get(id).then(setForm).catch(() => navigate('/secondaries'));
  }, [id, navigate]);
  ...
  const addOption = () => setForm({ ...form, scoringOptions: [...form.scoringOptions, { ...emptyOption }] });
  const removeOption = (index: number) => setForm({ ...form, scoringOptions: form.scoringOptions.filter((_, i) => i !== index) });
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" min={0} value={form.maxVp} onChange={(e) => setForm({ ...form, maxVp: parseInt(e.target.value) || 0 })} className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-sm" />
          </div>
          <div className="flex items-end pb-2">
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={form.isFixed} onChange={(e) => setForm({ ...form, isFixed: e.target.checked })} className="rounded" />
              Fixed Secondary
            </label>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button type="button" onClick={addOption} className="text-xs text-amber-400 hover:text-amber-300">+ Add Option</button>
          </div>
          {form.scoringOptions.map((opt, i) => (
            <div key={i} className="mb-2 p-3 bg-gray-800 rounded border border-gray-700">
              <div className="flex items-center justify-between mb-2">
                ...
                <button type="button" onClick={() => removeOption(i)} className="text-xs text-red-400 hover:text-red-300">Remove</button>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Label</label>
                  <input type="text" value={opt.label} onChange={(e) => updateOption(i, 'label', e.target.value)} className="w-full px-2 py-1 bg-gray-700 border border-gray-600 rounded text-xs" />
                </div>
                <div>
                  <label className="block text-xs text-gray-500 mb-1">VP</label>
                  <input type="number" value={opt.vp} onChange={(e) => updateOption(i, 'vp', parseInt(e.target.value) || 0)} className="w-full px-2 py-1 bg-gray-700 border border-gray-600 rounded text-xs" />
                </div>
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Mode</label>
                  <select value={opt.mode || ''} onChange={(e) => updateOption(i, 'mode', e.target.value)} className="w-full px-2 py-1 bg-gray-700 border border-gray-600 rounded text-xs">
          ...
          {form.scoringOptions.length === 0 && <p className="text-xs text-gray-500">No scoring options.</p>}
        </div>
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => navigate('/secondaries')} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded text-sm">Cancel</button>
        </div>
      </form>
    </div>
  );
