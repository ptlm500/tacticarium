...
export function SecondaryListPage() {
  ...
  const load = () => {
    ...
    adminApi.secondaries.list(params).then(setData).finally(() => setLoading(false));
  };
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => navigate('/secondaries/new')} className="px-3 py-1.5 text-sm bg-amber-600 hover:bg-amber-500 rounded">Create</button>
      </div>
      ...
      <DataTable
        columns={[
          ...
          { key: 'scoringOptions', label: 'Options', render: (s) => <span>{s.scoringOptions.length}</span> },
        ]}
        data={data}
        getKey={(s) => s.id}
        searchField="name"
        onEdit={(s) => navigate(`/secondaries/${s.id}/edit`)}
        onDelete={async (s) => { await adminApi.secondaries.delete(s.id); load(); }}
      />
    </div>
  );
}
