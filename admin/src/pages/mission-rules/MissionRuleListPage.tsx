...
export function MissionRuleListPage() {
  ...
  const load = () => {
    ...
    adminApi.missionRules.list(params).then(setData).finally(() => setLoading(false));
  };
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => navigate('/mission-rules/new')} className="px-3 py-1.5 text-sm bg-amber-600 hover:bg-amber-500 rounded">Create</button>
      </div>
      ...
      <DataTable
        ...
        onDelete={async (r) => { await adminApi.missionRules.delete(r.id); load(); }}
      />
    </div>
  );
}
