...
export function GambitEditPage() {
  ...
  const [form, setForm] = useState<Gambit>({ id: '', missionPackId: '', name: '', description: '', vpValue: 0 });
  ...
  useEffect(() => {
    if (id) adminApi.gambits.get(id).then(setForm).catch(() => navigate('/gambits'));
  }, [id, navigate]);
    ...
    <div className="p-6 max-w-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input type="number" min={0} value={form.vpValue} onChange={(e) => setForm({ ...form, vpValue: parseInt(e.target.value) || 0 })} className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-sm" />
        </div>
        <div>
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => navigate('/gambits')} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded text-sm">Cancel</button>
        </div>
      </form>
    </div>
  );
