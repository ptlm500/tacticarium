...
export function GambitListPage() {
  ...
  const load = () => {
    ...
    adminApi.gambits.list(params).then(setData).finally(() => setLoading(false));
  };
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => navigate('/gambits/new')} className="px-3 py-1.5 text-sm bg-amber-600 hover:bg-amber-500 rounded">Create</button>
      </div>
      ...
      <DataTable
        ...
        onDelete={async (g) => { await adminApi.gambits.delete(g.id); load(); }}
      />
    </div>
  );
}
