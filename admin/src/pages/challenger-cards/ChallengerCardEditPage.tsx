...
export function ChallengerCardEditPage() {
  const navigate = useNavigate();
  const { id } = useParams();
  const isEdit = Boolean(id);

  const [form, setForm] = useState<ChallengerCard>({ id: '', missionPackId: '', name: '', lore: '', description: '' });
  ...
  useEffect(() => {
    if (id) adminApi.challengerCards.get(id).then(setForm).catch(() => navigate('/challenger-cards'));
  }, [id, navigate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      if (isEdit) await adminApi.challengerCards.update(id!, form);
      else await adminApi.challengerCards.create(form);
      navigate('/challenger-cards');
    } catch (err) {
  ...
  return (
    <div className="p-6 max-w-lg">
      <h2 className="text-xl font-bold mb-4">{isEdit ? 'Edit' : 'Create'} Challenger Card</h2>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => navigate('/challenger-cards')} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded text-sm">Cancel</button>
        </div>
      </form>
    </div>
  );
